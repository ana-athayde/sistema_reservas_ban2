...
def get_all():
    ...
    .order_by(Reserva.id_pag, Pagamento.id_pagamento).all() # nao coloquei Passagem no order by nao sei como fazer ??
    # fazer outro order by? ou inlcuir no mesmo Reserva.id_passagem, Passagem.id_passagem

def insert(**kwargs):
    ins = postgresql.insert(Reserva).values(kwargs).on_conflict_do_update(index_elements=[Reserva.id_reserva], set_=kwargs)
