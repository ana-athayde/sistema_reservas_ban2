...
from app.dao import reserva_dao, passageiro_dao
...
def reserva_passageiro():
    reserva_passageiro_form = ReservaForm()
    if reserva_passageiro_form.validate_on_submit():
        reserva_dao.insert(**remove_csrf(reserva_passageiro_form.data))
    ...
    rows = reserva_dao.get_all()
    rows += passageiro_dao.get_all()
    table = [dict(row) for row in rows]
    return render_template('page.html', title='ReservaPassageiro', table=table, forms=[reserva_passageiro_form, passageiro_form])